    ...
    try:
        ...
        with open('db.txt') as db:
            if url not in db.read():
                def get_sub():
                    for word in python:
                        if word in title: return 'python'; break
                    for word in linux:
                        if word in title: return 'linux'; break
                    for word in programming:
                        if word in title: return 'programming'; break
                    for word in politics:
                        if word in title: return 'politics'; break
                    for word in internet:
                        if word in title: return 'internet'; break
                    for word in gaming:
                        if word in title: return 'gaming'; break
                if get_sub() is not None:
                    reddit.submit(get_sub, title, url)
                else: print('no sub found')
                with open('db.txt', 'a') as f: f.write(url + '\n')
            else: print('url is in database')
        time.sleep(60)
    except Exception: print(str(traceback.format_exc()));time.sleep(60)
